    ...
    export default {
      data() {
        return {
          ...
          isLoading: false,
          backgroundImages: [
            { url: 'warehouse.svg', description: 'Almacén Moderno' },
            { url: 'inventory.svg', description: 'Control de Inventario' },
            { url: 'shipping.svg', description: 'Logística y Distribución' },
            { url: 'supply-chain.svg', description: 'Cadena de Suministro' }
          ],
          currentBgIndex: 0,
          bgInterval: null
        };
        ...
        async login() {
          ...
          try {
            ...
            if (data.rol && data.rol.includes('ROLE_USER')) {
              ...
              this.$router.push({ name: "tomaino" }); // O la vista que corresponda
            }
