    ...
    <div class="confirmacion-container">
      ...
      <div v-else class="terms-list">
        ...
        <transition-group name="list-animation" tag="div" class="terms-cards">
          <div 
            v-for="termino in filteredTerminos" 
            :key="termino.id" 
            class="term-card animate__animated animate__fadeIn"
            :class="{'expanded': expandedId === termino.id, 'confirmed': termino.confirmado}"
            :data-id="termino.id"
          >
            ...
            <div class="term-body" v-show="expandedId === termino.id">
              <div class="term-details">
                ...
                <div class="detail-row animate__animated animate__fadeIn" style="animation-delay: 0.35s">
                  ...
                  <div class="detail-value">{{ termino.faltanteValor  || 'Sin valor' }}</div>
                </div>
                ...
                <div class="detail-row animate__animated animate__fadeIn" style="animation-delay: 0.4s">
                  ...
                  <div class="detail-value">{{ termino.sobranteValor  || 'Sin valor' }}</div>
                </div>
                ...
                <div class="detail-row animate__animated animate__fadeIn" style="animation-delay: 0.45s">
                  ...
                  <div class="detail-value">{{ termino.conciliadoValor || 'Sin valor' }}</div>
                </div>
                ...
                <div class="response-section animate__animated animate__fadeIn" style="animation-delay: 0.6s">
                  ...
                  <div v-if="!termino.confirmado" class="response-options">
                    <button 
                      ...
                      :class="{'selected': respuestas[termino.id] === 'Aceptado'}"
                      @click="seleccionarRespuesta(termino.id, 'Aceptado')"
                    >
                    ...
                    <button 
                      class="option-button reject" 
                      :class="{'selected': respuestas[termino.id] === 'No Aceptado'}"
                      @click="seleccionarRespuesta(termino.id, 'No Aceptado')"
                    >
                ...
                <div class="action-section animate__animated animate__fadeIn" style="animation-delay: 0.65s">
                  <button
                    v-if="!termino.confirmado"
                    @click="confirmar(termino.id)"
                    class="confirm-button"
                    :disabled="!respuestas[termino.id]"
                  >
                  ...
                  <button
                    v-else
                    @click="generarPDFIndividual(termino)"
  ...
  setup() {
    ...
    const confirmar = async (id) => {
      ...
      try {
        ...
        if (terminoIndex !== -1) {
          const updatedTermino = { ...terminos.value[terminoIndex] };
    ...
    const generarPDFIndividual = async (termino) => {
      ...
      try {
        ...
        doc.text('Confirmación de Término de Inventario', 105, 20, { align: 'center' });
        
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(12);
        doc.setTextColor(80, 80, 80);
        doc.text('Centro de Distribución', 105, 30, { align: 'center' });
        ...
        let yPos = 60;
        const detalles = [
          { label: 'ID', value: termino.id },
          { label: 'Fecha Aceptación', value: termino.fechaAceptacion },
          { label: 'Usuario ID', value: termino.usuarioId || 'Sin usuario' },
          { label: 'Centro', value: termino.centro || 'Sin centro' },
          { label: 'Faltante SKU', value: termino.faltanteSku || 'Sin Faltante SKU' },
          { label: 'Sobrante SKU', value: termino.sobranteSku || 'Sin Sobrante SKU' },
          { label: 'Conciliado SKU', value: termino.conciliadoSku || 'Sin Conciliado SKU' },
          { label: 'Faltante Valor', value: termino.faltanteValor || 'Sin valor' },
          { label: 'Sobrante Valor', value: termino.sobranteValor || 'Sin valor' },
          { label: 'Conciliado Valor', value: termino.conciliadoValor || 'Sin valor' }
        ];
        ...
        doc.text(`RESPUESTA: ${respuestas.value[termino.id]}`, 105, yPos + 5, { align: 'center' });
        ...
        doc.setTextColor(80, 80, 80);
        doc.setFontSize(10);
        doc.text(`Confirmado el: ${termino.fechaConfirmacion || new Date().toLocaleDateString('es-PE')}`, 105, yPos, { align: 'center' });
        
        // Pie de página
        doc.setFont('helvetica', 'italic');
        doc.setFontSize(8);
        doc.setTextColor(100, 100, 100);
        doc.text('Documento oficial de confirmación de inventario', 105, 285, { align: 'center' });
    ...
    const generarPDF = async () => {
      ...
      try {
        ...
        doc.text('Confirmación de Términos de Inventario', 105, 20, { align: 'center' });
        ...
        doc.setFontSize(10);
        doc.text(`Fecha: ${fechaActual}`, 195, 10, { align: 'right' });
        ...
        doc.setFontSize(14);
        doc.setTextColor(12, 83, 76);
        doc.text('Resumen de Confirmaciones', 105, yPosition, { align: 'center' });
        ...
        for (const termino of filteredTerminos.value) {
          ...
          if (termino.confirmado) {
            ...
            doc.setTextColor(255, 255, 255);
            doc.setFontSize(8);
            doc.text(respuestas.value[termino.id], 175, yPosition + 2, { align: 'center' });
          } else {
            ...
            doc.setTextColor(255, 255, 255);
            doc.setFontSize(8);
            doc.text('Pendiente', 175, yPosition + 2, { align: 'center' });
          }
...
.term-status-indicator {
  ...
  width: 100%;
  height: 4px;
  background-color: #f1c40f; /* Por defecto, pendiente */
}
...
.confirm-button:hover:not(:disabled), .download-button:hover {
  transform: translateY(-3px);
...
.confirm-button i, .download-button i {
  font-size: 1.1rem;
}

.confirm-button.loading, .download-button.loading {
  pointer-events: none;
}

.confirm-button.loading i, .download-button.loading i {
...
.option-button:active, .confirm-button:active, .download-button:active, .pdf-button:active {
