  ...
  export default {
    ...
    async buscarAsistencias() {
      ...
      try {
        ...
        // Priorizar la búsqueda por DNI si está presente
        if (this.filtroDni) {
          url = `http://localhost:8765/api/asistencia/empleado/${this.filtroDni}`;
        } 
        // Si no hay DNI pero hay estado, buscar por estado
        else if (this.filtroEstado) {
          url = `http://localhost:8765/api/asistencia/horas-extras/estado/${this.filtroEstado}`;
        } 
        // Si no hay filtros, cargar pendientes por defecto
        else {
          url = 'http://localhost:8765/api/asistencia/horas-extras/pendientes';
        }
