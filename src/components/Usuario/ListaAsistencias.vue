  ...
  export default {
    ...
    methods: {
      async buscarAsistencias() {
        ...
        this.cargando = true;
        try {
          const response = await axios.get(`http://localhost:8765/api/asistencia/empleado/${this.dniBusqueda}`);
      ...
      editarAsistencia(asistencia) {
        this.asistenciaEditando = { ...asistencia };
        this.mostrarModal = true;
      },
      
      async guardarEdicion() {
        try {
          await axios.put(`http://localhost:8765/api/asistencia/${this.asistenciaEditando.id}`, this.asistenciaEditando);
          alert('Asistencia actualizada correctamente');
          this.mostrarModal = false;
          this.buscarAsistencias(); // Refrescar la lista
        } catch (error) {
      ...
      async enviarSolicitudHorasExtras() {
        try {
          await axios.post(
            `http://localhost:8765/api/asistencia/${this.asistenciaSeleccionada.id}/horas-extras`, 
            this.horasExtras
          );
          alert('Solicitud de horas extras enviada correctamente');
          this.mostrarModalHorasExtras = false;
          this.buscarAsistencias(); // Refrescar la lista
        } catch (error) {
          console.error('Error al solicitar horas extras:', error);
          alert('No se pudo enviar la solicitud de horas extras');
        }
      }
    }
  };
